...
from app.core.workflow.dataset_synthesis.sampler import SimpleRandomSubGraphSampler, SubGraphSampler
...
class SamplingDatasetGenerator(DatasetGenerator):
    ...
    async def generate(self, task_desc: str, dataset_name: str, size: int) -> WorkflowTrainDataset:
        ...
        )  # 生成size个问题，需要 size // nums_per_subgraph 上取整轮，+20来控制最大失败次数
        times = 0
        ...
        while total < size and times < max_times:
            ...
            try:
                ...
                print(
                    f"[SamplingDatasetGenerator][generate] except while get_random_subgraph, reason={e}"
                )
                continue
