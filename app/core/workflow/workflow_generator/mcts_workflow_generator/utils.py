...
def format_yaml_with_anchor(text: str, key: str, need_anchor_name: bool = True, fields: List[str] = []) -> str:
    ...
    if need_anchor_name and len(anchors) != len(yaml_list):
        return ValueError(f"length of anchors（{len(anchors)} unmatch length of actions list {len(yaml_list)}")
...
async def generate_json( 
                   ...
                   max_retry: int = 3,
                   filter: Callable[[JsonValue],JsonValue] = lambda data: True
                   ) -> JsonValue:
        times = 0
        obj = None
        while times < max_retry:
            ...
            try:
                ...
                json_result = parse_jsons(resp_str)
                return filter(json_result)
            except Exception as e:
                print(f"[LLMExpander][generate_json] failed, reason={e}, times={times}")
                messages.append(
                    ModelMessage(
                        payload=resp_str,
                ...
                messages.append(
                    ModelMessage(
                        payload=f"When parse json encounter exception={e}, please output the right json format.",
