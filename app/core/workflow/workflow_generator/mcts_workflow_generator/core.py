...
from app.core.workflow.workflow_generator.mcts_workflow_generator.model import WorkflowLogFormat, AgenticConfigSection
...
from app.core.workflow.workflow_generator.mcts_workflow_generator.utils import load_config_dict, load_agentic_service
...
class MCTSWorkflowGenerator:
    def __init__(self, 
                 ...
                 optimize_grain: List[AgenticConfigSection] = [AgenticConfigSection.EXPERTS, AgenticConfigSection.OPERATORS],
                 max_rounds: int = 30, 
    ...
    def init_workflow(self):
        ...
        config_dict = self.load_config_dict(round_num=1)
        for section in AgenticConfigSection:
            section_name = str(section.value)
            section_context = config_dict.get(section_name)
            if section_context == None:
                print(f"[MCTSWorkflowGenerator][init_workflow] Cann't find {section_name} in  {workflow_file}") 
                continue
    ...
    def load_config_dict(self, round_num: int, skip_section: List[AgenticConfigSection] = []) -> Dict[str, str]:
    ...
    async def run(self):
        ...
        self.init_workflow()
        score, experience = self.evaluator.evaluate_workflow(round_num=1, dataset=test_data, modifications=[], optimized_path=self.optimized_path)
        self.logs[1] = WorkflowLogFormat(round_number=1, score=score, experience=experience, modifications=[], feedbacks=[])
        ...
        for round_num in range(2, self.max_rounds+1):
            ...
            # Load Workflow
            current_config = self.load_config_dict(select_round.round_number)

            # Expand the workflow
            new_flow = await self.expander.expand(
                ...
                with open(new_flow_path, 'w', encoding='utf-8')  as f:
                    for section in AgenticConfigSection:
                        if section not in self.optimize_grain:
                            ...
                            if section_init_context == None:
                                print(f"[MCTSWorkflowGenerator][run] Cann't find {section} in init_config_dict")
                                continue
                            f.write(section_init_context)
                            f.write("\n\n")
                    for _, section_context in new_flow.new_configs.items():
            ...
            score, experience = self.evaluator.evaluate_workflow(
                round_num=round_num, 
                dataset=train_data, 
                modifications=new_flow.modifications,
                optimized_path=self.optimized_path
            )

            # save result
            self.logs[round_num] = WorkflowLogFormat(round_number=round_num, score=score, experience=experience, modifications=new_flow.modifications, feedbacks=[])
