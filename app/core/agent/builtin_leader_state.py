...
class BuiltinLeaderState(LeaderState):
    """Builtin leader state with persistence support (Issue #40).
    ...
    def __init__(self):
        super().__init__()
        # Issue #40: Cache for expert configurations loaded from database
    ...
    def get_expert_by_id(self, expert_id: str) -> Expert:
        """Get existing expert instance with lazy loading (Issue #40)."""
    ...
    def create_expert(self, agent_config: AgentConfig) -> Expert:
        """Add an expert profile to the registry with persistence (Issue #40)."""
