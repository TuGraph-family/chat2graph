...
def get_data_importation_operator():
    ...
    schema_understanding_action = Action(
        ...
        name="图模型理解",
        description="调用相关工具获取图模型，并对图模型进行分析和理解。",
    )

    content_understanding_action = Action(
        id="data_generattion_and_import.content_understanding_action",
        name="文本内容理解",
        description="调用相关工具获取原始文本内容，并结合图模型进行分析和理解，目的是为了后续更好地从文本中提取出知识图谱。",
    )

    triplet_data_generation_action = Action(
        id="data_generattion_and_import.triplet_data_generation_action",
        name="三元组数据抽取",
        description="根据对图模型理解和文本内容理解的理解，进行三元组数据的抽取，并存入图数据库中。遇到‘optional: false’的属性必须进行数据补全，输出的格式要严格符合输出模板，不要输出其他无关信息。",
    )
    ...
    toolkit_service.add_action(
        ...
        next_actions=[(triplet_data_generation_action, 1)],
        prev_actions=[(schema_understanding_action, 1)],
    )

    toolkit_service.add_action(
        id=operator.get_id(),
        action=triplet_data_generation_action,
    ...
    toolkit_service.add_action(
        ...
        next_actions=[],
        prev_actions=[(triplet_data_generation_action, 1)],
    )

    toolkit_service.add_tool(
        id=operator.get_id(), tool=get_schema, connected_actions=[(schema_understanding_action, 1)]
    )
    ...
    toolkit_service.add_tool(
        id=operator.get_id(),
        tool=import_data,
        connected_actions=[(triplet_data_generation_action, 1)],
    )
    return operator
