...
| `add_tool_group(self, tool_set: ToolSetClient, connected_actions: List[Tuple[Action, float]]) -> None` | Adds a series of tools in the `tool_set` to the Toolkit. `tool_set` is the toolset client object to add, `connected_actions` is a list of `Action` objects invoking tools in this toolset with their associated scores. The method creates all `tool`s contained in the toolset and adds them to the toolkit while associating them with connected `action`s.                                              |
