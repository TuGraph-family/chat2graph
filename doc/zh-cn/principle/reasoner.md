...
| 模型参数                        | 说明                                                                                   |
| --------------------------------- | -------------------------------------------------------------------------------------- |
| MODEL_PLATFORM_TYPE               | 指定所用模型平台类型（DB-GPT、AiSuite、LiteLLM 等）。推荐使用兼容 OpenAI API 的模型。         |
| LLM_NAME                          | 大语言模型的名称，例如 `gemini-2.0-flash`、`geminie-2.5-flash`、`o3-mini` 等（也可能是 `opneai/gemini-2.5-flash`，表示 OpenAI API 兼容，具体格式要根据模型平台适配）。                                   |
...
| MAX_COMPLETION_TOKENS             | 限制模型每次生成的最大 completion token 数。                                           |
