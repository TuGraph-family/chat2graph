...
class DataImport(Tool):
    ...
    @staticmethod
    def validate_and_clean_graph(graph: Dict[str, Any]) -> Dict[str, Any]:
        """
        验证并清理图数据，确保其符合预期的结构。
        """
        ...
        print(f"[log] cleaned_graph data: {cleaned_graph}")
        return cleaned_graph

    async def import_data(self, graph: Dict[str, Any]):
        """
        Import the graph data into the database.

        Args:
            graph (Dict[str, Any]): The graph data to be imported. It should contain 'entities' and 'relationships' keys.
                - 'entities': A list of dictionaries, each representing an entity with 'label' and 'properties'.
                - 'relationships': A list of dictionaries, each representing a relationship with 'label', 'constraints', 'source', 'target', and 'properties'.

        Returns:
            Dict[str, Any]: A dictionary containing the results of the import operations.
                - 'entities_import_result': The result of importing entities.
                - 'edges_import_result': The result of importing relationships.
        """
        # 验证并清理图数据
        ...
        entities_import_result = db.conn.run(cypher_statement)
        ...
        for edge_type, edges_list in edge_types.items():
            ...
            edges_import_result.append(res)
        
        return {
            "entities_import_result": entities_import_result,
            "edges_import_result": edges_import_result,
        }
...
def get_data_generation_and_import_operator():
    ...
    edge_data_generation_action = Action(
        ...
        name="关系数据抽取",
        description=f"根据对图模型理解和文本内容理解的理解，进行关系数据的抽取，关系的数量不能少于{COUNT}条，输出的格式要严格符合输出模板，不要输出其他无关信息。关系数据抽取完成后，进行下一步'实体数据抽取'操作。",
    )

    node_data_generation_action = Action(
        id="data_generattion_and_import.node_data_generation_action",
        name="实体数据抽取",
        description="根据关系数据，图模型和文本内容，进行实体数据的抽取，输出的格式要严格符合输出模板，不要输出其他无关信息。实体数据抽取完成后，进行下一步'整合实体和关系数据'操作。",
    )
    ...
    import_data_action = Action(
        ...
        name="导入实体和关系数据",
        description="调用数据导入工具，将json数据转换成导入命令，并执行导入。数据导入成功后，进行下一步'输出结果'操作。",
    )
    ...
    import_data = DataImport(id="data_impor_tool")
    analysis_toolkit.add_tool(
