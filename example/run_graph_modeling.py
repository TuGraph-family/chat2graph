...
class DocumentReader(Tool):
    ...
    def __init__(self, id: Optional[str] = None):
        super().__init__(id=id, function=self.read_document)
...
class SchemaGenerator(Tool):
    """Tool for generating GQL schema definitions."""

    def __init__(self, id: Optional[str] = None):
        super().__init__(id=id, function=self.generate_schema)

    async def generate_schema(
        self, entity_types: List[Dict], relation_types: List[Dict]
    ) -> str:
        """Generate GQL schema /label definitions for the TuGraph DB. The generator
...
2. 关系类型
3. 约束条件
4. 总结一下，最终的 GQL 指令是什么？
请注意，你生成的是 GQL 指令，让 TuGraph DB 建立 Graph Schema（label），而不是数据导入的 GQL 指令。 
这个指令将会给TuGraph DB执行 GQL。同时，你不需要提到“你我他”这样的人称代词，直接回答我给你的任务就好，也不要说那些不相关的话。
        """
...
class SchemaValidator(Tool):
    ...
    def __init__(self, id: Optional[str] = None):
        super().__init__(id=id, function=self.validate_schema)

    async def validate_schema(self, schema: str) -> str:
        """Validate schema definition.
        If the schema is valid, return the validation results. Otherwise, return the error message.


        Args:
            schema (str): GQL schema definition
        ...
        prompt = """假设你是 TuGraph DB 的管理员，请验证我给你 GQL 指令的正确性。
无论你是否验证通过，都应该给出信息反馈。同时，请确保，输入是用于 Create Schema 的 GQL 指令，而不是数据导入的 GQL 指令。
        """
...
def get_analysis_operator():
    ...
    operator_config = OperatorConfig(
        instruction=DOC_ANALYSIS_INSTRUCTION,
        profile=DOC_ANALYSIS_PROFILE,
    ...
    operator = Operator(config=operator_config, id="analysis_operator")
...
def get_concept_modeling_operator():
    ...
    operator_config = OperatorConfig(
        instruction=CONCEPT_MODELING_INSTRUCTION,
        profile=CONCEPT_MODELING_PROFILE,
        output_schema=CONCEPT_MODELING_OUTPUT_SCHEMA,
        toolkit=concept_modeling_toolkit,
        actions=[
    ...
    operator = Operator(config=operator_config, id="concept_modeling_operator")
